...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        #compute adjoint first(i.e. the gradient of loss w.r.t this node)
        node_adj = None
        for out_grad in node_to_output_grads_list[node]:
            if node_adj is None:
                node_adj = out_grad
            else:
                node_adj = node_adj + out_grad    
        node.grad = node_adj #save adjoint into grad field
        #if there is no operation associated with this node, then we have reached input tensor
        if node.op is None:
            continue
        #compute gradient from this node to all previous nodes 
        pre_to_nodes = list(node.op.gradient_as_tuple(node_adj,node))
        for i, pre in enumerate(node.inputs):
            if pre not in node_to_output_grads_list.keys():
                node_to_output_grads_list[pre]=[pre_to_nodes[i]]
            else:
                node_to_output_grads_list[pre].append(pre_to_nodes[i])
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, topo_order):
    ...
    for pre in node.inputs:
        if pre not in topo_order:
            topo_sort_dfs(pre,topo_order)
