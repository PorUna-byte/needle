...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if not self.is_compact():
            raise ValueError("Not compact")
        out = NDArray.make(new_shape, device=self.device, handle=self._handle)
        return out
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        new_shape = []
        new_strides = []
        assert len(new_axes) == len(self.shape), "permute error:the new axis length doesn't match the origin shape"
        for _, s in enumerate(new_axes):
            new_shape.append(self.shape[s])
            new_strides.append(self.strides[s])
        out = NDArray.make(shape=tuple(new_shape),strides=tuple(new_strides),device=self.device, handle=self._handle)
        return out
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert all([self.shape[i] == new_shape[i] or self.shape[i] ==1] for i in range(self.ndim))
        broadcast_axis=[]
        for i in range(self.ndim):
            if self.shape[i] == 1 and new_shape[i] > 1:
                broadcast_axis.append(i)
        new_strides = list(self.strides)
        for k in broadcast_axis:
            new_strides[k]=0
        new_strides = new_strides + [0]*(len(new_shape)-self.ndim)
        out = NDArray.make(shape=new_shape, strides=tuple(new_strides), device=self.device, handle=self._handle)
        return out
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        out = NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=offset)
        return out
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        return NDArray.make(shape=self.shape,strides=tuple(strides),device=self.device, handle=self._handle,offset=offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        padded_shape = [dim+axes[i][0]+axes[i][1] for i,dim in enumerate(self.shape)]
        slice_idx = [slice(axes[i][0],axes[i][0]+dim) for i,dim in enumerate(self.shape)]
        padded_array = full(shape=padded_shape,fill_value=0,device=self.device)
        padded_array[tuple(slice_idx)] = self
        return padded_array
        ### END YOUR SOLUTION
